...
bool LevelGameScene::init() {
	...
	backItem->setTarget(this, menu_selector(LevelGameScene::onClickBtn));
	backItem->setTag(-1);
	...
	resetItem->setTarget(this, menu_selector(LevelGameScene::onClickBtn));
	resetItem->setTag(-2);
	...
	for (int i = 0; i < ROW_NUM; i++) {
		for (int j = 0; j < COL_NUM; j++) {
            ...
            item->setTag(index);
            item->setTarget(this, menu_selector(LevelGameScene::onClickGrid));
    ...
    _winNode = CCNode::create();
    addChild(_winNode, 11);
    CCSprite* winBg = CCSprite::create(Images::game::dialogBg_win);
    _winNode->setContentSize(winBg->getContentSize());
    ...
    LayoutUtil::layoutToParentCenter(_winNode, this, 0, -10);
    _winNode->setVisible(false);
    
    
    _loseNode = CCNode::create();
    addChild(_loseNode, 11);
    CCSprite* loseBg = CCSprite::create(Images::game::dialogBg_lose);
    _loseNode->setContentSize(loseBg->getContentSize());
...
void LevelGameScene::detectDrops(){
...
void LevelGameScene::showDropWall(int startIndex, int rotation){
    ...
    wallDrop->runAction(CCSequence::create(animate, CCCallFuncN::create(this, callfuncN_selector(LevelGameScene::clearNode)), NULL));
    _remainClearNode++;
}

int LevelGameScene::getGridIndexByPos(cocos2d::CCPoint & pos){
...
void LevelGameScene::onExplodeEnd(WaterSprite *water){
    ...
    for (int i = 0; i < 4; i++) {
        ...
        drop->runAction(CCSequence::create(move, CCCallFuncN::create(this, callfuncN_selector(LevelGameScene::removeDrop)), NULL));
        
        _dropArrays->addObject(drop);
    }
}

bool LevelGameScene::hasWater(){
    return _remainWaterNum > 0;
}

bool LevelGameScene::isGridValid(int index){
...
void LevelGameScene::clearNode(cocos2d::CCNode *node){
    node->removeFromParent();
    _remainClearNode--;
}

void LevelGameScene::onClickGrid(cocos2d::CCNode *node){
...
void LevelGameScene::onClickBtn(CCNode* node) {
	int tag = node->getTag();
...
void LevelGameScene::nextStage(cocos2d::CCNode *node){
    
    int totalCount = LevelUtil::getLevelStageCount(_curLevel);
    CCLOG("total %d, cur %d", _curStage, totalCount);
    if(_curStage + 1 > totalCount){
        
        int nextLevel = _curLevel + 1;
        
        nextLevel = nextLevel <= LevelUtil::getLevelCount() ? nextLevel : _curLevel;
        
        SelectStageScene* selectStage = SelectStageScene::create(nextLevel);
        CCTransitionFade* trans = CCTransitionFade::create(Constants::REPLACE_SCENE_TIME, selectStage);
        
        CCDirector::sharedDirector()->replaceScene(trans);
    }else{
        CCLOG("next stage level %d, stage %d", _curLevel, _curStage + 1);
        LevelGameScene* classicScene = LevelGameScene::create();
        classicScene->setData(_curLevel, _curStage + 1);
        CCTransitionFade* trans = CCTransitionFade::create(Constants::REPLACE_SCENE_TIME, classicScene);
        
        CCDirector::sharedDirector()->replaceScene(trans);
        
        LevelUtil::setUserCurLevelStage(_curLevel, _curStage + 1);
    }
}

void LevelGameScene::retryStage(cocos2d::CCNode *node){
    CCLOG("retry");
    LevelGameScene* classicScene = LevelGameScene::create();
    classicScene->setData(_curLevel, _curStage);
    CCTransitionFade* trans = CCTransitionFade::create(Constants::REPLACE_SCENE_TIME, classicScene);
